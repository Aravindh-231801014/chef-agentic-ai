...
def evaluate(reference, generated):
    ...
    print("\n" + "="*60, flush=True)
    print("         🌟 AI GENERATION QUALITY AUDIT 🌟", flush=True)
    print("="*60, flush=True)
    print(f" 📊 BLEU Score:    {results['BLEU']}", flush=True)
    print(f" 📊 ROUGE-1:       {results['ROUGE-1']}", flush=True)
    print(f" 📊 ROUGE-L:       {results['ROUGE-L']}", flush=True)
    print("-" * 60, flush=True)
    print(f" 🛡️  Bias Score:    {results['Bias']}  (1.0 = Perfect)", flush=True)
    print(f" 🤝 Fairness:      {results['Fairness']}  (1.0 = Perfect)", flush=True)
    print(f" 🎯 Faithfulness:  {results['Faithfulness']}  (1.0 = Perfect)", flush=True)
    print(f" 🔍 Factuality:    {results['Hallucination']}  (1.0 = No Hallucination)", flush=True)
    print("="*60 + "\n", flush=True)
